...
-type storage_type() :: array:array('prime' | sets:set(pos_integer()) | 'undef').
...
-spec process_number(Number :: pos_integer(), Count :: pos_integer(), Storage :: storage_type()) -> pos_integer().
...
first(SourceList, Count) ->
    first(SourceList, 0, Count, []).
...
first([SourceHead | SourceListRest], Index, Count, Dest) ->
    first(SourceListRest, Index + 1, Count, [SourceHead] ++ Dest).

-spec get_prime_dividers(Number :: pos_integer(), Storage :: storage_type()) -> sets:set(pos_integer()).
get_prime_dividers(Number, Storage) -> get_prime_dividers(Number, 2, Storage).

-spec get_prime_dividers(Number :: pos_integer(), Divider :: pos_integer(), Storage :: storage_type()) -> sets:set(pos_integer()).
get_prime_dividers(Number, Divider, _Storage) when Divider * Divider > Number -> prime;
get_prime_dividers(Number, 2, Storage) ->
    Rem = Number rem 2,
    if
        Rem == 0 -> sets:add_element(2, get_dividers(Number div 2, Storage));
        Rem /= 0 -> get_prime_dividers(Number, 3, Storage)
    end;
get_prime_dividers(Number, Divider, Storage) ->
    case array:get(Divider - 2, Storage) of
        prime ->
            Rem = Number rem Divider,
            if
                Rem == 0 -> sets:add_element(Divider, get_dividers(Number div Divider, Storage));
                Rem /= 0 -> get_prime_dividers(Number, Divider + 2, Storage)
            end;
        _Other -> get_prime_dividers(Number, Divider + 2, Storage)
    end.

-spec get_dividers(Number :: pos_integer(), Storage :: storage_type()) -> sets:set(pos_integer()).
...
-spec prepare_dividers(Number :: pos_integer(), MinNumber :: pos_integer(), Storage :: storage_type()) -> storage_type().
...
-spec check_number(StartNumber :: pos_integer(), Index :: non_neg_integer(), Count :: pos_integer(), Storage :: storage_type()) ->
    {Result :: boolean(), Number :: pos_integer(), Storage :: storage_type()}.
...
-spec check_dividers(Dividers :: 'prime' | sets:set(pos_integer()), Count :: pos_integer()) -> boolean().
